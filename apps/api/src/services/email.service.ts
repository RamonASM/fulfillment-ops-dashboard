...
const EMAIL_FROM = process.env.EMAIL_FROM || 'Inventory IQ <noreply@inventoryiq.com>';
...
export async function initializeEmailService(): Promise<void> {
  ...
  try {
    ...
    logger.error('SMTP connection failed', error as Error, { host: process.env.SMTP_HOST });
  }
}
...
export async function sendLowStockAlert(
  ...
  const criticalCount = products.filter(p => p.status === 'CRITICAL' || p.status === 'STOCKOUT').length;
...
export async function sendOrderStatusUpdate(
  ...
  const statusMessages: Record<string, { subject: string; message: string; alertClass: string }> = {
    APPROVED: {
      subject: 'Your Order Request Has Been Approved',
      message: 'Great news! Your order request has been approved and is being processed.',
      alertClass: 'alert-info',
    },
    REJECTED: {
      subject: 'Your Order Request Was Not Approved',
      message: 'Unfortunately, your order request could not be approved at this time.',
    ...
    CHANGES_REQUESTED: {
      ...
      message: 'Your order request needs some revisions before it can be processed. Please review the notes and update your request.',
      alertClass: 'alert-warning',
    },
    ACKNOWLEDGED: {
      subject: 'Your Order Request Has Been Acknowledged',
      message: 'Your order request has been received and is being reviewed by our team.',
      alertClass: 'alert-info',
    },
  };
...
export async function sendWeeklyDigest(
  ...
  const issueRows = digest.topIssues.map(i => `
    <tr>
      <td>${i.client}</td>
...
async function sendEmail(
  ...
  if (!transporter) {
    logger.warn('Email transporter not initialized, skipping email', { to, subject });
    return;
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      logger.debug('Email preview available', { previewUrl: nodemailer.getTestMessageUrl(info) });
    }
  } catch (error) {
